...
class Aes : public Crypto {
    ...
    uint8_t roundKey8[11][16];

    // AES Round Constant
    const uint32_t rc[10] = {0x01000000, 0x02000000, 0x04000000, 0x08000000, 0x10000000, 0x20000000, 0x40000000, 0x80000000, 0x1b000000, 0x36000000};
    
    // AES Subbytes(and inverse) table
    ...
    uint32_t getu(uint8_t *state); // u8[4] -> u32
    void putu(uint8_t *ct, uint32_t state); // u32 -> u8[4]

    // AES Encryption functions
    void encryptRound(uint32_t state[4], int round);

    // AES Decryption functions
    void addRoundKey8(uint8_t state[16], uint8_t rRoundKeyPrime[16]);
    void invSubBytes(uint8_t state[16]);
    void invShiftRows(uint8_t state[16]);
    void invMixColumns(uint8_t state[16]);

public:
    Aes(uint8_t *key, uint8_t *pt, int keyLen, int ptLen): Crypto(key, pt, keyLen, ptLen) { }
